...
#
# PyInstaller 构建配置 - Windows XP 兼容版单文件 EXE
# 输出: dist/SensitiveDetector.exe
# 特点: 无需安装 Python，双击即用，不写入注册表，兼容 Windows XP
#
# 兼容 XP 关键要求:
#   1. Python 3.8 (需使用非官方 XP 兼容构建, 如 wangwenx190 的版本)
#   2. PyInstaller 4.x (引导程序 bootloader 支持 XP, 5.x/6.x 引导程序要求 Win7+)
#   3. 32 位架构 (XP 主要是 32 位系统)
#   4. Everything 1.4.x (1.5.x 不支持 XP)
#
# 构建方式（二选一）:
...
a = Analysis(
    ...
    excludes=excludes,
    noarchive=False,
    # 注意: module_collection_mode 是 PyInstaller 6.x 新增参数, 4.x 不支持
    # 已移除该参数以兼容 PyInstaller 4.x (XP 引导程序支持)
)

pyz = PYZ(a.pure, a.zipped_data, cipher=block_cipher)

# ---------------------------------------------------------------------------
# 单文件 EXE（非目录模式，便携无毒，兼容 Windows XP）
# 注意: disable_windowed_traceback 和 argv_emulation 是 PyInstaller 5.x+ 新增参数
# 已移除以兼容 PyInstaller 4.x (XP 引导程序支持)
# ---------------------------------------------------------------------------
exe = EXE(
    pyz,
    a.scripts,
