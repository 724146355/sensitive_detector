...
Windows XP 兼容性:
  - Everything 1.4.x 支持 XP，1.5.x 不支持 XP
  - 请确保 XP 系统上安装 Everything 1.4.x 版本
  - DLL 加载时已通过 hasattr 兼容旧版 SDK 函数（IsFolderResult、GetResultSize 等）
"""
import ctypes
import os
import sys
...
class EverythingScanner:
    ...
    def _load_dll(self):
        ...
        self.logger.info("Everything 未安装或 SDK DLL 未找到（XP 用户请确保安装 Everything 1.4.x）")
